...
ifeq ($(shell uname), Darwin)
	...
	brew update
	brew install mpfr libmpc gh git libomp yaml-cpp gmp gsl cmake boost openjdk@17 git-lfs xxhash tbb antlr4-cpp-runtime gcc
	# $(MAKE) android-setup-mac
else ifeq ($(shell uname), Linux)
	if [ -f /etc/fedora-release ]; then \
		sudo dnf install -y libomp yaml-cpp gmp-devel gsl-devel cmake boost-devel; \
	else \
		sudo apt-get install -y make build-essential libssl-dev zlib1g-dev libbz2-dev libreadline-dev libsqlite3-dev wget curl llvm libncursesw5-dev xz-utils tk-dev libxml2-dev libxmlsec1-dev libffi-dev liblzma-dev openjdk-21-jdk libxxhash-dev lldb libtbb-dev pybind11-dev; \
